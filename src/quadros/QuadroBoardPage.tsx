...
function MiniCard({ c, onClick, onSoltarAntes, expandidas, onToggleEt, usuariosMap, ehPost }: {
  ...
  // Capa explícita (cor ou imagem) vs. 1ª imagem só anexada (sem ser capa).
  const capaExplicita = c.capa || '';
  const ehCorCapa = capaEhCor(capaExplicita);
  const imgCapa = capaExplicita && !ehCorCapa ? capaExplicita : '';
  const imgAnexo = !capaExplicita
    ? ((c.anexos ?? []).find((a) => (a.mime ?? '').startsWith('image') && a.url)?.url ?? '')
    : '';
  const temArte = ehCorCapa || !!imgCapa || !!imgAnexo;
  ...
  return (
    ...
    >
      {ehCorCapa && (
        <div style={{ background: capaExplicita }} className="relative h-24 w-full">
          <p className={cn('absolute inset-x-0 bottom-0 line-clamp-2 px-2.5 pb-2 text-base font-semibold leading-snug', capaCorClara(capaExplicita) ? 'text-zinc-900' : 'text-white')}>{c.nome}</p>
        </div>
      )}
      {imgCapa && (
        // Capa explícita: banner recortado (compacto).
        <div className="relative w-full">
          <img
            src={thumbUrl(imgCapa)}
            alt=""
            loading="lazy"
            decoding="async"
      ...
      {imgAnexo && (
        // Imagem só anexada (sem ser capa): mostra a imagem INTEIRA (card maior).
        <div className="relative w-full">
          <img
            src={thumbUrl(imgAnexo)}
            alt=""
            loading="lazy"
            decoding="async"
          ...
          />
          <div className="pointer-events-none absolute inset-x-0 bottom-0 h-1/3 bg-gradient-to-t from-black/85 via-black/40 to-transparent" />
          <p className="absolute inset-x-0 bottom-0 line-clamp-2 px-2.5 pb-2 text-base font-semibold leading-snug text-white drop-shadow-[0_1px_2px_rgba(0,0,0,0.8)]">{c.nome}</p>
        </div>
      )}
      <div className="flex flex-col gap-1.5 p-2.5">
        ...
        {!temArte && <p className="text-base leading-snug">{c.nome}</p>}
