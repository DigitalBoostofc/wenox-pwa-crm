...
import { Plus, X, Building2, Users, ClipboardList } from 'lucide-react';
import {
...
/** Formulário completo de tarefa — reutilizado na página inteira e no
 *  cadastro inline (painel que expande na lista). Não navega: avisa o
 *  pai via onSalvo / onApagado / onCancelar. */
export function TarefaForm({
  tarefaId, presetProjeto, presetCliente,
  onSalvo, onApagado, onCancelar,
}: {
  tarefaId?: string;
  ...
  // Pré-seleção (preset de projeto/cliente) ao criar.
  useEffect(() => {
    if (tarefaId) return;
    if (souCliente) {
      setModo('cliente');
    ...
    if (presetProjeto || presetCliente) {
      setModo('cliente');
      setForm((f) => ({
        ...f, projeto: presetProjeto ?? '', cliente: presetCliente ?? '', lado: 'cliente',
      }));
    }
    setCarregado(true);
  }, [tarefaId, presetProjeto, presetCliente, souCliente, user?.cliente]);

  useEffect(() => {
    if (!tarefaId) return;
    getTarefa(tarefaId).then((t) => {
      setForm({
        nome: t.nome,
  ...
  function trocarModo(m: Modo) {
    ...
    setForm((f) => ({
      ...
      projeto: '', cliente: '', contato: '', responsaveis: [],
    }));
  }
  ...
  async function salvar(e: React.FormEvent) {
    ...
    try {
      ...
      const rec = tarefaId
        ? await atualizarTarefa(tarefaId, payload)
        ...
        <CardContent className="flex flex-col gap-4">
          {souCliente && (
            <p className="text-xs text-muted-foreground">
              A tarefa fica vinculada à sua empresa. Escolha o projeto, se quiser.
            </p>
          )}
          {!souCliente && (
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      ativo ? 'border-primary/50 bg-primary/10' : 'border-border hover:bg-secondary',
                    )}
                  >
                    <span className={cn(
          ...
          {modo !== 'interna' && form.cliente && (
            <Campo id="proj" label={souCliente ? 'Projeto' : `${modo === 'equipe' ? '3' : '2'}. Projeto`}>
              {projetosDoCliente.length === 0 ? (
                <p className="rounded-md border border-dashed border-border px-3 py-2 text-xs text-muted-foreground">
                  Este cliente ainda não tem projetos. A tarefa fica ligada só ao cliente.
                </p>
              ) : (
          ...
          {!souCliente && modo === 'cliente' && form.cliente && (
            <Campo id="contato" label="3. Contato responsável (do cliente)">
              {contatos.length === 0 ? (
                <p className="rounded-md border border-dashed border-border px-3 py-2 text-xs text-muted-foreground">
                  Nenhum contato cadastrado para este cliente. Cadastre na aba Equipe do cliente.
                </p>
              ) : (
